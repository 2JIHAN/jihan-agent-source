...
사용자가 이 규칙에 대한 피드백을 줄 때, 변경 사항은 이 SKILL.md 파일을 직접 update 한 뒤 `git commit && git push` 한다. 다른 머신에서는 `claude plugin update agent-doc-rules` 로 반영. 로컬 메모리, 에이전트 메모리, CLAUDE.md 에 저장하지 않는다.
