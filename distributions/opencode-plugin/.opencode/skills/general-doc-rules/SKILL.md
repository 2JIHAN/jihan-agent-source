...
- 번호 목록, 글머리 기호 (bullet) 항목이 명사/체언으로 끝나면 마침표 (온점) 를 붙이지 않는다 (예 `1. Homebrew 설치 확인`, `- 로컬 런타임`). 동사형 종결로 끝나는 항목 (`~한다`, `~된다`, `~합니다`) 만 마침표를 붙인다. 이 규칙은 일반 문서, 방법 문서를 가리지 않고 모든 문서 산출물에 적용한다.
...
- 명령어를 나열할 때는 가능한 한 하나의 코드블록에 묶고, 각 명령 오른쪽에 `# 설명` 인라인 주석으로 용도를 적는다. 명령마다 별도 블록 + 위에 한 줄 설명을 두는 스타일은 피한다.
- 같은 블록 안에서 주석은 컬럼을 맞춰 정렬해 위에서 아래로 훑기 쉽게 한다.
- 섹션 (`#` 헤더) 단위로 묶음 블록을 둔다. 한 섹션 안에 같은 도구의 관련 명령들을 모은다.
- 단계별 절차 문서 (`method-doc-rules`) 의 단계 명령은 예외로, 단계 설명 바로 아래 단일 명령 블록을 유지한다.

예시.

```bash
docker images             # 전체 이미지 목록
docker images -a          # 중간 레이어까지 포함
docker images -q          # ID만 출력
docker image ls           # docker images 와 동일
```
...
사용자가 이 규칙에 대한 피드백을 줄 때, 변경 사항은 `source/skills/general-doc-rules/SKILL.md` 를 직접 update 한 뒤 `scripts/sync-distributions.sh`, `git commit && git push` 순서로 반영한다. Claude Code 와 OpenCode 배포물은 `source/` 에서 생성된 사본으로 취급한다. 로컬 메모리, 에이전트 메모리, CLAUDE.md 에 저장하지 않는다.
